...
describe("migration chain (with mock migrations)", () => {
  ...
  test("applies single migration", () => {
    // Add a migration from 0.15.0 → 0.16.0; the real contiguous migrations
    // are already in originalMigrations and complete the path to SCHEMA_VERSION.
  ...
  test("applies migration chain in order", () => {
    ...
    MIGRATIONS.push(
      ...
      ...originalMigrations, // include the real chain to complete the path
    );
  ...
  test("throws when migration fails", () => {
    ...
    expect(() => migrateIfNeeded({}, "0.15.0")).toThrow(
      /Migration 0\.15\.0 → 0\.16\.0 failed/,
    );
  });
  ...
  test("throws when no migration path exists", () => {
    ...
    expect(() => migrateIfNeeded({}, "0.5.0")).toThrow(
      /No migration path from 0\.5\.0/,
    );
  });
});
...
describe("v0.16.0 → v0.19.0 migrations", () => {
  ...
  test("v0.17.0 data passes through the no-op migration unchanged", () => {
    const legacyData = {
      settings: { sort: "descending", previewTags: true },
      pages: [
        {
          images: [
            { id: "a1", path: "/legacy", name: "legacy", type: "image" },
          ],
        },
      ],
    };
    const result = migrateIfNeeded(legacyData, "0.17.0");
    expect(result.migrated).toBe(true);
  ...
  test("full v0.16.0 → v0.19.0 chain keeps legacy attachments descriptor-absent", () => {
    const legacyData = {
      ...
      pages: [
        {
          files: [{ id: "a1", path: "/legacy", name: "legacy", type: "file" }],
        },
      ],
    };
    const result = migrateIfNeeded(legacyData, "0.16.0");
    const settings = (result.data as Record<string, unknown>)
      .settings as Record<string, unknown>;
    const pages = (result.data as Record<string, unknown>).pages as Array<
