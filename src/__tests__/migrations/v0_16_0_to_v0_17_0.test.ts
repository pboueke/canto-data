...
describe("v0_16_0_to_v0_17_0 migration", () => {
  ...
  it("does not modify page count", () => {
    const journal = makeV0_16_Journal();
    const pagesBefore = (
      (journal as Record<string, unknown>).pages as unknown[]
    ).length;
    migrate(journal);
    const pagesAfter = ((journal as Record<string, unknown>).pages as unknown[])
      .length;
  ...
  it("preserves syncProvider and themeOverride", () => {
    const journal = makeV0_16_Journal();
    const settings = (journal as Record<string, unknown>).settings as Record<
      string,
      unknown
    >;
  ...
  it("preserves unknown extra fields on settings (forwards compat)", () => {
    ...
    const result = migrate(journal) as Record<string, unknown>;
    expect((result.settings as Record<string, unknown>).futureField).toBe(
      "should-survive",
    );
  });
  ...
  it("preserves unknown extra fields on pages (forwards compat)", () => {
    const journal = makeV0_16_Journal();
    const pages = (journal as Record<string, unknown>).pages as Record<
      string,
      unknown
    >[];
  ...
  it("does not corrupt data when caller holds a reference to settings", () => {
    ...
    expect(settingsRef as Record<string, unknown>).not.toHaveProperty(
      "showMarkdownPlaceholder",
    );
  });
